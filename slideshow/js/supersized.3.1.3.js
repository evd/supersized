...
(function($){
	...
	$.supersized = function( options ) {
		...
		var settings = {
			...
			fit_image				:   0,		//All images will not exceed browser width or height
		...
		function resizenow() {
			return element.each(function() {
		  		...
		  		$(t).each(function(){
					...
					var max_width = (options.fit_image || (options.fit_landscape && ratio<=1))?browserwidth:0;
					var min_height = options.min_height;
					var max_height = (options.fit_image || (options.fit_portrait && ratio>1))?browserheight:0;

					var newwidth = browserwidth;
					if (newwidth<options.min_width)
						newwidth = options.min_width;
					if (max_width>0 && newwidth>max_width)
						newwidth = max_width;
					var newheight = newwidth*ratio;
					if (newheight<options.min_height) {
						newheight = options.min_height;
						newwidth = newheigh/ratio;
					}
					...
					thisSlide.width(newwidth);
					thisSlide.height(newheight);
